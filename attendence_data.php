...
$query = "SELECT * FROM attendence ORDER BY date DESC, name ASC";
    ...
    <div class="col-md-9 col-lg-10 main-content">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
          ...
          <a href="add_atendence.php" class="btn btn-primary">
            <i class="fas fa-plus"></i> Add Attendance
          </a>
        </div>
        <h2 class="text-center flex-grow-1">Employee Attendance Records</h2>
      </div>
          ...
          <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
              <tr>
                ...
                <td><?= $row['out_time'] ?? '—' ?></td>
                <td>
                  <a href="edit_attendance_all.php?name=<?= urlencode($row['name']) ?>&date=<?= urlencode($row['date']) ?>" class="btn btn-warning btn-sm">
